...
c.E={size:3};vm.createContext(c);vm.runInContext(fs.readFileSync(__dirname+'/operation-help.js','utf8'),c);
...
c.state[0].face='B';c.queue=[];frames.shift()();assert.equal(node('operationStep').textContent,'4 / 7 · そろえて攻撃');
assert.ok(node('operationPrompt').textContent.includes('3個'));
node('operationNext').click();assert.equal(node('operationStep').textContent,'5 / 7 · チェイン');
node('operationNext').click();assert.equal(node('operationStep').textContent,'6 / 7 · バースト');
node('operationNext').click();assert.equal(node('operationExit').textContent,'遊ぶ');assert.equal(node('operationNext').hidden,true);
