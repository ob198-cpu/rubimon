...
const original=[{id:1,face:'R',locked:2,p:[1,1,1]}],saved=JSON.stringify(original),frames=[];let normalCalls=0,pointerDown;
const c={document:{createElement:()=>node('new'+nodes.size),querySelector:node,querySelectorAll:()=>[node('hint'),node('rescue')],body:node('body'),addEventListener:(type,fn,capture)=>{assert.equal(type,'pointerdown');assert.equal(capture,true);pointerDown=fn}},byId:node,canvas:node('canvas'),state:original,viewYaw:.7,viewPitch:.4,active:null,queue:[],tutorial:null,panelPick:null,phase:'ready',structuredClone,refresh(){},updateView(){},requestAnimationFrame:fn=>frames.push(fn),userMove:()=>normalCalls++};
...
pointerDown({preventDefault(){throw Error('touch must not close before completion')},stopImmediatePropagation(){}});assert.ok(c.stickLesson);
...
let blocked=false;pointerDown({preventDefault(){blocked=true},stopImmediatePropagation(){blocked=true}});
assert.equal(blocked,true,'dismissal touch must not operate the board beneath the panel');assert.equal(c.state,original);assert.equal(JSON.stringify(c.state),saved);assert.equal(c.stickLesson,null);assert.equal(node('hint').inert,false);
