...
for(const size of [3,4,5]){E.configure(size);for(const tile of E.create()){
 ...
 for(const [direction,vector] of Object.entries({up:[0,-30],down:[0,30],left:[-30,0],right:[30,0]})){
  const move=c.stickMoveFor(tile,...vector),selected=c.ringSelectedMove(tile,direction);assert.equal(move.face,selected.face);
...
for(const d of ['up','down','left','right'])assert.equal(c.ringSelectedMove(null,d),null);
assert.ok(src.includes('b.disabled=!(picked&&can&&move&&B.canRotate'));
assert.ok(!src.includes('data-axis'),'no row/column mode switch');
assert.ok(!stick.includes('ringAllowsMove'),'trigger and keyboard accept all directions');
